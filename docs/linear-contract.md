...
The configured Linear workspace and team are the source of truth for requirements, priority, approval, and status display. The local filesystem is authoritative for claims, retries, reservations, and crash recovery. Workspace and team bindings are supplied explicitly to `loopctl init`; Workloop contains no organization-specific defaults.
